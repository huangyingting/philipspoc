...
Packs solution from PR merge, creates release, and deploys to production.
...
flowchart TD
  ...
  subgraph Release[release-on-merge-to-prod-cn]
    B1[Checkout] --> B2[Extract metadata]
    B2 --> B3[Pack solution]
    B3 --> B4[Upload artifact]
    B4 --> B5{Tag exists?}
    B5 -->|No| B6[Create tag & release]
    B6 --> B7{Import?}
    B7 -->|Yes| B8[Deploy to prod]
  end
  A --> B1
  classDef trig fill:#0366d6,stroke:#fff,color:#fff
```
...
**Triggers**: PR merge to main | `workflow_dispatch`

**Metadata Source**: PR body (extracts from HTML comment block)

**Safety**: 
- `IMPORT_ENABLED`: Controls production deployment
- Concurrency group: `release-main`
- Tag duplicate check

### Workflow

1. **Export** → Creates PR with metadata (export-and-branch-solution-cn)
2. **Review** → Merge PR
3. **Auto-deploy** → Pack, tag, release, import
...
### All Workflows
- **Cloud**: China (Mooncake)
- **Auth**: Service principal (app registration)
- **Actions**: microsoft/powerplatform-actions

### Permissions
- `contents: write` (tags, releases, branches)
- `pull-requests: write` (PR operations)

### Folder Structure
```
solutions/           # Unpacked solutions
├── <SolutionName>/  # Solution components
out/
...
### Version Format
- Semantic: `X.X.X.X`
- Tags: `v` prefix (e.g., `v1.2.3.4`)
- Auto-strip `v` for Dataverse

### Best Practices
1. Use export-and-branch for changes
2. Review PR before merge
3. Let automation handle releases
4. Keep `IMPORT_ENABLED` false until ready
- Pull Request: Automated PR with:
  - Title: "Power platform managed export: <SolutionName> v<Version>"
  - Body: Description with embedded metadata block
  - Files: All unpacked solution components

### Permissions Required
- contents: write (for branch creation)
- pull-requests: write (for PR creation/update)
